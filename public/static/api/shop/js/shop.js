...
var shop_obj = {
    ...
    tao_detail_init: function() {		
        ...
        function() {
            ...
            var qty = parseFloat($('#qty_selector input[name=Qty]').val()) - 1;
            if (qty < 1) {
                qty = 1;
            }
            var internel = parseInt($('#coins').val());
            var coins_str = '';
            if(internel){
                coins_str = parseInt(internel * qty);
            }
            $('#price').html(parseFloat(okprice * qty));
            $('#product_coins').html(coins_str);
        ...
        function add_to_cart(cart_key) {
            ...
            $.post($('#addtocart_form').attr('action') + 'ajax/', $('#addtocart_form').serialize()+'&cart_key='+cart_key+'&atr_str='+attrvstr+'&atrid_str='+attridstr+'&price='+price+'&pricesimp='+pricesimp+'&showimg='+showimg+'&count='+countarr,
            function(data) {
                if (data.status == 1) {
    ...
    cart_init: function() {
        ...
        //选择购物车中的产品
        $('.select_cart_product').click(function () {
            var BizID = $(this).attr("BizID");
            var ProductsIDs = [];//定义一个空数组
            var CartIDs = [];//定义一个空数组
            $('.item').find("input:checkbox:checked").each(function(i){//把所有被选中的复选框的值存入数组
                ProductsIDs[i] =$(this).val();
                CartIDs[i] =$(this).attr('CartID');
            });
            $.post(ajax_url, 'action=cart_select&BizID=' + BizID + '&ProductsIDs=' + ProductsIDs + '&CartIDs=' + CartIDs,
                function(data) {
        ...
        /*$('#cart .bizcart .item .del div').click(function() {
			// if(!window.confirm("很抢手哦~下次不一定能买到\n\n确认要删除我吗~")) return false;
            var BizID = $(this).attr("BizID");
            var ProductsID = $(this).attr("ProductsID");
            var CartID = $(this).attr("CartID");
            var obj = $(this);
			// var msg = "很抢手哦~下次不一定能买到\n\n确认要删除我吗~";
        ...
        $('#cart .bizcart .item .info .price span i').click(function() { //更新购物车            
            ...
            function(data) {
                if (data.status == 2) { //更新购物车成功                    
                    ...
                    var all_txt = data.total;
                    if (data.total_coin != 0) {
                        all_txt = all_txt + ' + ' + data.total_coin + '积分';
                    }
					// $('#cart .cart_total span').html(all_txt);
                } else if (data.status == 1) {
        ...
        $('#cart .cart_total a.gotocheck').click(function() {
            ...
            $.post(ajax_url, 'action=cart_check&ProductsIDs='+ProductsIDs+'&CartIDs='+CartIDs,
            function(data) {
